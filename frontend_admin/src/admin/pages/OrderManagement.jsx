...
const OrderManagement = () => {
  ...
  const [storeId, setStoreId] = useState(""); // 🔥 lọc cửa hàng

  /* ================= FETCH ORDERS ================= */
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        let url = `${API_BASE}/api/orders/admin/orders/system`;
        if (storeId) {
          url += `?storeId=${storeId}`;
        }
  ...
  const updateStatus = async (id, status) => {
    try {
      await axios.put(
        `${API_BASE}/api/orders/admin/orders/status/${id}`,
        { status },
        {
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    <div className="order-container">
      ...
      <div className="order-filters">
        <select value={storeId} onChange={(e) => setStoreId(e.target.value)}>
          ...
          <option value={process.env.REACT_APP_STORE_ID_1}>
            {process.env.REACT_APP_STORE_NAME_1}
          </option>
          <option value={process.env.REACT_APP_STORE_ID_2}>
            {process.env.REACT_APP_STORE_NAME_2}
          </option>
        </select>
          ...
          <tbody>
            {filteredOrders.map((order) => (
              <tr key={order._id}>
                ...
                <td className="address">{order.shippingInfo.address}</td>
                <td>
      ...
      {selectedOrder && (
        <div className="modal-overlay">
          <div className="modal">
            ...
            <p><b>Mã đơn:</b> {selectedOrder.orderCode}</p>
